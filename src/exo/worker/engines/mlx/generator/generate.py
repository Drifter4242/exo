...
def make_logits_processors_from_task(
    ...
    """
    # Convert logit_bias from OpenAI format (str keys, int values) to mlx_lm format (int keys, float values)
    ...
    if processors:
        logger.info(
            f"Created logits processors: logit_bias={logit_bias is not None}, repetition_penalty={repetition_penalty}"
        )
